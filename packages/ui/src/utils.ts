...
export function createVariants<T extends Record<string, any>>(config: T) {
  return function variants(props: Partial<keyof T extends string ? Record<keyof T, boolean> : never>) {
...
export function resolveResponsiveValue<T>(
  ...
  if (typeof value === "object" && value !== null && !Array.isArray(value) && "mobile" in value) {
    const responsiveObj = value as { mobile?: T; tablet?: T; desktop?: T };
    return responsiveObj[breakpoint] ?? responsiveObj.desktop ?? responsiveObj.tablet ?? responsiveObj.mobile ?? (value as any);
  }
  return value as T;
}
