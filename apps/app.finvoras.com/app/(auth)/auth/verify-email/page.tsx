...
function VerifyEmailContent() {
  ...
  const handleResend = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setResendMessage(error instanceof Error ? error.message : content.resendFailed);
    }
  };
    ...
    return (
      <Card className="w-full shadow-2xl border-0 bg-white dark:bg-card/50 backdrop-blur-xl animate-in fade-in zoom-in duration-300">
        <CardHeader className="text-center pb-2">
          ...
          <p className="text-sm text-muted-foreground mt-2">{content.resendSubtitle}</p>
        </CardHeader>
        <CardContent className="space-y-4">
          {resendMessage && (
            <div className={`p-3 rounded-lg text-sm text-center ${resendStatus === 'success' ? 'bg-green-50 text-green-600 dark:bg-green-900/20' : 'bg-red-50 text-red-600 dark:bg-red-900/20'}`}>
              {resendMessage}
            </div>
          )}
          ...
          <form onSubmit={handleResend} className="space-y-4">
            ...
            <Button type="submit" className="w-full" disabled={resendStatus === 'loading' || resendStatus === 'success'}>
              {resendStatus === 'loading' ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : null}
              {resendStatus === 'success' ? content.sent : content.sendLink}
            </Button>
          </form>

          <Button variant="link" className="w-full text-sm text-muted-foreground" onClick={() => setShowResend(false)}>
            {content.back}
          </Button>
        </CardContent>
      </Card>
    ...
    <Card className="w-full shadow-2xl border-0 bg-white dark:bg-card/50 backdrop-blur-xl animate-in fade-in zoom-in duration-500 overflow-hidden">
      <div className={`h-2 w-full ${status === "success" ? "bg-green-500" :
        status === "error" ? "bg-red-500" :
          status === "loading" ? "bg-brand-primary animate-pulse" : "bg-muted"
        }`} />

      <CardContent className="pt-10 pb-8 px-8 flex flex-col items-center text-center gap-6">
        {/* State Icon */}
        <div className="relative">
          <div className={`w-24 h-24 rounded-full flex items-center justify-center transition-all duration-500 ${status === "success" ? "bg-green-100 text-green-600 dark:bg-green-900/20" :
        ...
        <div className="space-y-3 max-w-xs">
          <h1 className="text-2xl font-bold tracking-tight">
            {status === "loading" ? content.verifying :
              status === "success" ? content.successTitle :
                status === "error" ? content.errorTitle :
                  content.title}
          </h1>

          <p className="text-muted-foreground leading-relaxed text-sm">
            {status === "success" ? content.successMessage :
              status === "error" ? errorMessage || content.noToken : // Show backend error message
                status === "idle" ? content.subtitle :
