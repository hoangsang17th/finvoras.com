...
export function RegisterForm({
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		if (!formData.email.includes("@"))
			newErrors.email = t.auth.register.validEmailRequired;
			...
			<CardContent className="space-y-6">
				...
				<form onSubmit={handleSubmit} className="space-y-4">
					<div className="space-y-2">
						...
						/>
						{errors.fullName && (
							<p className="text-sm text-brand-red">{errors.fullName}</p>
						)}
					</div>
					...
					<div className="space-y-2">
						...
						/>
						{errors.email && (
							<p className="text-sm text-brand-red">{errors.email}</p>
						)}
					</div>
					...
					<div className="space-y-2">
						...
						{errors.password && (
							<p className="text-sm text-brand-red">{errors.password}</p>
						)}
					</div>
					...
					<div className="space-y-2">
						...
						{errors.confirmPassword && (
							<p className="text-sm text-brand-red">{errors.confirmPassword}</p>
						)}
					</div>
					...
					<div className="space-y-4">
						<div className="flex items-start space-x-3">
							<input
								...
								onChange={(e) => {
									...
									setFormData((prev) => ({
										...prev,
										acceptLegalDocuments: e.target.checked,
									}));
								}}
							...
							<Label
								htmlFor="terms"
								className="text-sm text-brand-neutral-600 leading-5"
							>
						...
						{errors.terms && (
							<p className="text-sm text-brand-red">{errors.terms}</p>
						)}
					</div>
