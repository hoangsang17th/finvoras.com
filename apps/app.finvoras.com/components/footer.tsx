...
const Footer = ({ variant = "full", className = "" }: FooterProps) => {
	if (variant === "minimal") {
		return (
			<footer
				className={`py-8 px-6 border-t bg-background/50 backdrop-blur-sm ${className}`}
			>
				<div className="max-w-6xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4 text-sm text-muted-foreground font-medium">
					<div className="flex items-center gap-2">
						...
						<span className="hidden sm:inline">
							Built with heart for financial freedom
						</span>
					</div>

					<div className="flex items-center gap-6">
						<Link
							href="https://finvoras.com/privacy"
							className="hover:text-brand-primary transition-colors"
						>
							Privacy
						</Link>
						<Link
							href="https://finvoras.com/terms"
							className="hover:text-brand-primary transition-colors"
						>
							Terms
						</Link>
						<Link
							href="https://finvoras.com/contact"
							className="hover:text-brand-primary transition-colors"
						>
							Support
						</Link>
	...
	return (
		<footer
			className={`dark:border-t mt-40 dark bg-background text-foreground ${className}`}
		>
