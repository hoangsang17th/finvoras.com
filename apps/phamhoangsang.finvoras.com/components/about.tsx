...
import { MapPin, Heart, GraduationCap, Award, Calendar } from "lucide-react";
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              {personalInfo.bio.split('\n\n').map((paragraph: string, index: number) => {
                ...
                return (
                  <p key={index} className="text-base text-muted-foreground/90 leading-relaxed">
                    {paragraph.trim()}
                  </p>
                );
              })}
          ...
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-2 gap-6">
              {quickInfo.map((info: QuickInfo, index: number) => (
                <Card key={index} hoverable className="p-6 text-center shadow-sm">
                  <div className="text-3xl font-bold text-brand-primary mb-2 line-clamp-1" title={String(info.value)}>{info.value}</div>
                  <div className="text-sm text-muted-foreground font-medium mb-1">{info.label}</div>
                  {info.description && (
                    <div className="text-xs text-muted-foreground/80 leading-snug">{info.description}</div>
                  )}
                </Card>
              ))}
            </div>
            ...
            {resumeData.education && resumeData.education.length > 0 && (
              <div className="space-y-6">
                <div className="flex items-center gap-3">
                  ...
                  <h3 className="text-2xl font-bold">{ui.sections.education}</h3>
                </div>
                ...
                <Timeline
                  ...
                  items={resumeData.education.map((edu: Education) => ({
                    id: edu.id,
                    content: (
                      <Card hoverable className="p-6 border-none bg-gradient-to-br from-card to-muted/50 overflow-hidden relative">
                        ...
                        <div className="relative z-20">
                          <div className="flex flex-col md:flex-row md:items-start justify-between gap-3 mb-4">
                            <div className="min-w-0 flex-1">
                              <div className="flex items-center gap-2">
                                <h4 className="text-lg font-bold truncate" title={edu.institution}>
                                  {edu.institutionUrl ? (
                                    <TextLink href={edu.institutionUrl} external>
                                      {edu.institution}
                                    </TextLink>
                                  ) : (
                                    edu.institution
                          ...
                          {edu.gpa && (
                            <div className="flex items-center gap-2 mb-4 text-[11px] font-medium">
                              ...
                              <span className="text-muted-foreground uppercase tracking-widest opacity-70">GPA:</span>
                              <span className="text-foreground/90 font-bold">{edu.gpa}</span>
                            </div>
                          )}
