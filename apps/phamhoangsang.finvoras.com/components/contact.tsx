...
import type { UITranslations } from "@/lib/types/resume";
...
const ContactInfo = ({
	personalInfo,
	ui,
}: ContactInfoProps & { ui: UITranslations }) => {
...
const SuccessMessage = ({
	onReset,
	ui,
}: { onReset: () => void; ui: UITranslations }) => (
	...
	<form className="space-y-6" onSubmit={onSubmit}>
		<div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
			<div className="space-y-2">
				<Label htmlFor="name">
					{ui.ui.form.name}{" "}
					<span className="text-destructive">{ui.ui.form.contactRequired}</span>
				</Label>
				<Input
					id="name"
		...
		{submitStatus === "error" && (
			<div className="p-4 bg-destructive/10 border border-destructive/20 rounded-md">
				<p className="text-sm text-destructive">{ui.ui.actions.errorMessage}</p>
			</div>
		)}
			...
			<div className="py-20 px-6 bg-muted/30">
				<div className="max-w-6xl mx-auto">
					...
					<div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
						{/* Contact Information */}
						<ContactInfo personalInfo={personalInfo} ui={ui} />
