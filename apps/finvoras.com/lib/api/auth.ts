...
class ApiClient {
	...
	async register(userData: RegisterRequest): Promise<void> {
		...
		await this.httpClient.post<void>(API_ENDPOINTS.AUTH.REGISTER, formData);
	}
	...
	async logout(): Promise<void> {
		...
		try {
			await this.httpClient.delete<void>(API_ENDPOINTS.AUTH.LOGOUT, {
				params: { refreshToken },
			});
		} catch (error) {
	...
	async verifyEmail(token: string): Promise<void> {
		await this.httpClient.put<void>(API_ENDPOINTS.AUTH.VERIFY_EMAIL, null, {
			params: { token },
		});
	}

	async resendVerificationEmail(email: string): Promise<void> {
		await this.httpClient.post<void>(API_ENDPOINTS.AUTH.RESEND_EMAIL_VERIFY, {
			email,
		});
	}

	async forgotPassword(email: string): Promise<void> {
		await this.httpClient.post<void>(API_ENDPOINTS.AUTH.FORGOT_PASSWORD, {
			email,
		});
	}
	...
	async requestAccountReactivation(email: string): Promise<void> {
		await this.httpClient.post<void>(API_ENDPOINTS.AUTH.RE_ACTIVE, { email });
	}

	async reactivateAccount(token: string): Promise<void> {
		await this.httpClient.put<void>(API_ENDPOINTS.AUTH.RE_ACTIVE, null, {
			params: { token },
		});
	}
