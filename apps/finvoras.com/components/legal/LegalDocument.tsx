...
export function LegalDocument({
	...
	const [document, setDocument] = useState<LegalDocumentResponse | null>(
		initialDocument,
	);
					...
					<div className="pt-4 space-y-4">
						<div className="p-4 bg-muted/50 rounded-xl border border-border inline-flex items-center gap-3 text-sm text-foreground/80">
							...
							<span>
								...
								<a
									href="mailto:support@finvoras.com"
						...
						<div className="sticky top-24 space-y-8 max-h-[calc(100vh-120px)] flex flex-col">
							...
							<div className="pb-6 border-b border-border">
								<Dropdown
									...
									options={(document?.context?.supportedLanguages || []).map(
										(code) => ({
											value: code,
							...
							<nav className="space-y-4 overflow-y-auto pr-2 custom-scrollbar">
								<p className="text-xs font-bold uppercase tracking-widest text-muted-foreground opacity-50 sticky top-0 bg-background py-1">
									{t.legal.onThisPage}
								</p>
								...
								<header className="mb-12 space-y-4">
									...
									{document.summary && (
										<div className="mt-8 p-6 bg-muted/30 rounded-2xl border border-border italic text-muted-foreground leading-relaxed">
											<span className="font-semibold text-foreground not-italic block mb-2">
												{t.legal.summary}
											</span>
											{document.summary}
										</div>
					...
					<div className="absolute left-0 top-0 bottom-0 w-80 bg-background border-r border-border shadow-2xl animate-in slide-in-from-left flex flex-col">
						...
						<nav className="flex-1 space-y-8 overflow-y-auto custom-scrollbar px-6 pb-24">
							<div className="space-y-4">
								<p className="text-xs font-bold uppercase tracking-widest text-muted-foreground opacity-50 text-center">
			...
			{!isSidebarOpen && (
				<div className="lg:hidden fixed bottom-24 left-6 z-[10001] animate-in slide-in-from-bottom duration-500">
					...
					>
						...
						<span className="text-sm font-bold tracking-tight text-foreground uppercase tracking-widest text-[10px]">
							{t.legal.contents}
						</span>
					</button>
				</div>
