...
const FeatureComparison = () => {
    ...
    const featuresByCategory = Object.entries(featureComparison.categories).map(([key, categoryName]) => ({
        key,
        name: categoryName,
        ...
        <div className="py-16 px-6 bg-muted/30">
            <div className="max-w-7xl mx-auto">
                ...
                <div className="text-center mb-12">
                    <h2 className="text-3xl md:text-4xl font-bold mb-4">{featureComparison.title}</h2>
                    <p className="text-xl text-muted-foreground max-w-2xl mx-auto">{featureComparison.subtitle}</p>
                </div>
                    ...
                    {featuresByCategory.map((category) => (
                        <div key={category.key} className="border-b last:border-b-0">
                            ...
                            <div className="px-6 py-3 bg-muted/30">
                                <h3 className="font-semibold text-sm text-brand-primary">{category.name}</h3>
                            </div>
                            ...
                            {category.features.map((feature, idx) => (
                                ...
                                >
                                    ...
                                    <div className="flex justify-center">{renderFeatureValue(feature.family)}</div>
                                </div>
                            ))}
                        </div>
                    ))}
                    ...
                    {featureComparison.features.map((feature, idx) => (
                        <div key={idx} className="bg-background rounded-xl border p-4">
                            ...
                            <div className="grid grid-cols-3 gap-3 text-center">
                                ...
                                <div>
                                    <div className="text-xs text-muted-foreground mb-2">Family</div>
