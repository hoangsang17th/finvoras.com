...
export default function BlogPage() {
  ...
  return (
    <PageLayout className="min-h-screen bg-background">
      {/* Header */}
      <div className="bg-gradient-to-b from-brand-primary/10 via-background to-brand-success/5 py-16 px-6">
        <div className="max-w-6xl mx-auto text-center">
          <h1 className="text-4xl md:text-5xl font-bold mb-4 text-foreground">
            Financial Education Hub
          </h1>
          <p className="text-xl text-brand-neutral-600 dark:text-brand-grey-350 mb-8 max-w-2xl mx-auto">
            Learn practical financial skills to build wealth, manage money, and achieve your financial goals.
          </p>

          {/* Search Bar */}
          <div className="relative max-w-md mx-auto">
            <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-brand-neutral-500 h-4 w-4" />
            <Input
              type="search"
              placeholder="Search articles..."
              className="pl-10 pr-4 py-2 w-full focus:border-brand-primary"
            />
          </div>
        </div>
      </div>
          ...
          <div className="flex flex-wrap gap-3">
            {mockCategories.map((category) => (
              <Link key={category.id} href={`/blog/category/${category.slug}`}>
                <Badge variant="secondary" className="px-4 py-2 hover:bg-brand-primary hover:text-white transition-colors">
